...
- AIDT·LMS 접속로그, 장애시간, 사용빈도는 현재 총점에 넣지 않는다.
...
- "AIDT·LMS 자료 없이 실제 디지털교과서 활용 안정성을 판단했다."
...
## 6. 2단계 확장진단: AIDT·LMS 활용 안정성

### 설계 원칙

AIDT·LMS 로그는 좋은 지표지만 공개 공공데이터로 보기 어렵다. 따라서 현재 MVP 총점에는 반영하지 않고, 자료 제공 시 별도 리포트로 붙인다.
...
- `/schools/[id]`: `2단계 확장진단` 섹션에 AIDT·LMS 활용 안정성 슬롯 표시
...
4. 설명 카드
   ...
   - 본문: 공개자료형 지수와 AIDT·LMS 확장진단 분리 설명
...
4. 확장: AIDT·LMS 로그 제공 시 별도 운영 안정성 진단
...
| 보완 우선 | 30~44점 | 구간 배지 |
| 현행 유지 | 30점 미만 | 구간 배지 |
| 공개자료형 점수 | AIDT·LMS 미포함 1단계 점수 | 상세 리포트 |
| 데이터 신뢰도 | 판단 확실성 배지 | 상세 리포트, 데이터 페이지 |
| 2단계 확장진단 | 추가자료 제공 시 별도 진단 | 상세 리포트, 데이터 페이지 |
| AIDT·LMS 활용 안정성 | 접속·장애·사용빈도 기반 확장지표 | 상세 리포트, 데이터 페이지 |
...
flowchart TD
  ...
  E --> G["지원 우선순위 목록"]
  E --> H["학교 상세 리포트"]
  I["AIDT·LMS 접속로그·장애시간·사용빈도"] -. "추가자료 제공 시" .-> J["2단계 운영 안정성 진단"]
  J -. "별도 리포트" .-> H
```
...
| AIDT·LMS 로그 | 현재 미연계 | 운영 안정성 확장지표 |
...
- [ ] AIDT·LMS를 현재 총점에서 제외한 이유가 납득되는가
...
- AIDT·LMS: 총점 제외, 2단계 확장진단 슬롯으로 표시

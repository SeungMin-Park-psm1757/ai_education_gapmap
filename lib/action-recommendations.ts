...
export function getAdminActions(score: ReadinessScore): AdminAction[] {
  if (isFieldCheckFirst(score)) {
    return [
      ...
      {
        title: "자료 보완",
        why: "핵심 지표 일부가 직접자료보다 대리지표에 의존합니다.",
  ...
  if (score.raw.aiLearningOpportunity >= 55) {
    actions.push({
      title: "프로그램 연계",
      why: "공개자료상 AI·SW 학습기회 보강 신호가 큽니다.",
      action: "방과후 AI·SW 프로그램 또는 지역 SW교육센터 연계를 검토하세요.",
  ...
  if (score.raw.regionalAccess >= 55) {
    actions.push({
      title: "현장 확인",
      why: "학교 밖 자원 접근 신호가 약하게 나타납니다.",
      action: "온라인 공동수업, 거점학교, 외부 강사 파견 등 학교 밖 자원 연계를 검토하세요.",
  ...
  if (!actions.length) {
    actions.push(
      {
        title: "프로그램 연계",
        why: "공개자료상 긴급 신호는 낮지만 지속 모니터링이 필요합니다.",
  ...
  const defaults: AdminAction[] = [
    {
      title: "예산 검토",
      why: "지원 소요 지수는 예산 검토 대상을 좁히는 1차 신호입니다.",
      action: "지원 소요 지수와 학교 의견을 함께 보고 예산 배정 우선순위를 검토하세요.",
      requiredData: "학교 의견서, 예산 신청 내역"
    },
    {
      title: "현장 확인",
      why: "공개자료와 현장 상황이 다를 수 있습니다.",
      action: "교육지원청 현장 확인 또는 학교 추가자료 요청을 검토하세요.",
      requiredData: "현장 점검 기록, 추가 제출자료"
    }
  ];
